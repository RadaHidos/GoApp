        ...
        {trips.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-black/40 font-medium max-w-[200px] mx-auto">
                Start planning your first adventure now.
              </p>
            </div>
            <button
              onClick={() => router.push("/trips/new")}
              className="px-8 py-4 bg-[#1D1D1F] text-white font-black rounded-[20px] shadow-xl active:scale-95 transition-all"
            >
              Plan a new trip
            </button>
          </div>
              ...
              return (
                <motion.div
                  ...
                  className={`relative p-6 rounded-[28px] bg-white border cursor-pointer active:scale-[0.98] transition-all shadow-sm group ${
                    isNew
                      ? "border-[#8E7AF6] ring-4 ring-[#8E7AF6]/10"
                ...
                >
                  {isNew && (
                    <motion.div
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="absolute -top-3 -right-3 bg-[#8E7AF6] text-white text-[11px] font-black px-3 py-1.5 rounded-full shadow-lg z-10"
                    >
                      JUST SAVED
                    </motion.div>
                  )}
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="text-[22px] font-black tracking-tight leading-none mb-1">
                        {trip.destination}
                      </h3>
                      <p className="text-[13px] font-bold text-black/40 uppercase tracking-wide">
                        {trip.days} Days • {trip.people}{" "}
                        {trip.people === 1 ? "Person" : "People"}
                      </p>
                    </div>
                    <div className="text-right">
                      <span className="block text-[20px] font-black tracking-tighter">
                        €{Math.round(trip.totalCost)}
                      </span>
                      <span
                        className={`text-[11px] font-black uppercase tracking-wide px-2 py-0.5 rounded-md inline-block ${trip.isOverBudget ? "bg-red-100 text-red-500" : "bg-[#8E7AF6]/10 text-[#8E7AF6]"}`}
                      >
                        {trip.isOverBudget
                          ? "Over Budget"
                          : `Saved €${Math.round(trip.savings)}`}
                      </span>
                    </div>
                  </div>

                  {/* Additional Trip Details */}
                  <div className="flex items-center gap-4 py-3 border-t border-black/5 mb-3">
                    <div className="flex flex-col">
                      <span className="text-[10px] uppercase font-black text-black/20 tracking-wider">
                        Created
                      </span>
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[13px] font-bold text-black/60">
                        {trip.selectedFlight?.airline || "N/A"}
                      </span>
                    </div>
                    <div className="w-px h-8 bg-black/5" />
                    <div className="flex flex-col">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {trip.vibes.slice(0, 3).map((vibe) => (
                      <span
                        key={vibe}
                        className="text-[11px] font-bold bg-gray-50 border border-black/5 px-3 py-1.5 rounded-full text-black/60"
                      >
                        {vibe}
                      </span>
                    ))}
